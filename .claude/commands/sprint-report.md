...
item-reference format, ISO date ranges). The template below is `/sprint-report`-specific
(Markdown file output for stakeholders, stakeholder-first Executive Summary, emoji-heavy
for presentation readability, metrics dashboard, what-was-delivered grouped by system).
