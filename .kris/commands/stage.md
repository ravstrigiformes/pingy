...
```
dev (local) → staging (test data) → beta (live data) → main (production)
```

| Branch | Purpose | Data | Access |
|--------|---------|------|--------|
| `dev` | Active development, parallel agents | Test | Developers |
| `staging` | QA testing, lint/format validation | Test | QA Team |
| `beta` | Pre-production testing | Live | Beta testers |
| `main` | Production | Live | All users |
