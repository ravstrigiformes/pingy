...
| **Base branch** | `main` (production) | `dev` |
| **Branch prefix** | `hotfix/` | `bugfix/` or `feature/` |
| **Merge flow** | `main` → backport to `beta` → `staging` → `dev` | `dev` → `staging` → `beta` → `main` |
| **All branches updated** | Yes, automatically (backport) | No, flows through promotion (`/stage`) |
| **Auto-merge** | Yes, to main + backport PRs | No |
...
```
hotfix/* ──► PR ──► main ──► backport ──► beta ──► backport ──► staging ──► backport ──► dev
                      │                     │                      │                       │
                 (production)          (pre-prod)              (QA sync)              (dev sync)
                      │                     │                      │                       │
                   ALL 4 BRANCHES GET THE FIX (main first, then downstream)
```

**Key difference from `/fix`:** Hotfix goes directly to `main` (production) first, then backports downstream. Regular `/fix` flows through `dev` → `staging` → `beta` → `main` via `/stage`.
